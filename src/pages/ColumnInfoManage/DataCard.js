...
export default class DataCard extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      type: 'columnInfoManage/loadForm',
      payload: {
        id,
        callback,
      },
  ...
  onModalCancelClick = () => {
    this.props.dispatch({
      type: 'columnInfoManage/changeFormVisible',
      payload: false,
    });
    this.props.callback();
  };
  ...
  formSubmit = () => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        formData.Kind = 1;
        this.props.dispatch({
          type: 'columnInfoManage/submitDesc',
          payload: formData,
        });
      }
    });
  ...
  showExtra = () => {
    const {
      columnInfoManage: { formData, column },
      form,
    } = this.props;
  ...
  render() {
    const {
      columnInfoManage: { formTitle, formVisible, formData, submitting, listmode },
