...
export default class ShopInfoEdit extends PureComponent {
  state = { checked: true };
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (this.state.checked) {
          ...
          formData.legal_representative_idcard_on = values.legal_representative_idcard_on[0];
          formData.legal_representative_idcard_under = values.legal_representative_idcard_under[0];
          formData.pra_qua_certificate = values.pra_qua_certificate[0];
          formData.logo = values.logo[0];
          formData.business_license = values.business_license[0];
            ...
            <Row gutter={32}>
              <Col {...col}>
                <Form.Item label="统一社会信用代码">
                  {getFieldDecorator('uniform_social_creditcode', {
                    initialValue: formData.uniform_social_creditcode || '',
                    rules: [
                      {
                        required: true,
                        message: '请输入',
              ...
              <Col {...col}>
                <Form.Item label="经营单位类型">
                  {getFieldDecorator('business_unit_type', {
                    initialValue: formData.business_unit_type || '',
                    rules: [
                      {
                        required: false,
                        message: '请选择',
              ...
              <Col {...col}>
                <Form.Item label="法定代表人">
                  {getFieldDecorator('legal_representative', {
                    initialValue: formData.legal_representative || '',
                    rules: [
                      {
                        required: true,
                        message: '请输入法定代表人',
            ...
            <Row gutter={32}>
              <Col {...col}>
                <Form.Item label="法定代表人身份证(正面)">
                  <span style={{ color: 'red' }}>（图片上传格式jpg,jpeg,png）</span>
                  {getFieldDecorator('legal_representative_idcard_on', {
                    initialValue: formData.legal_representative_idcard_on
                      ? [formData.legal_representative_idcard_on]
                      : '',
                    rules: [
                      {
                        required: true,
              ...
              <Col {...col}>
                <Form.Item label="法定代表人身份证(反面)">
                  <span style={{ color: 'red' }}>（图片上传格式jpg,jpeg,png）</span>
                  {getFieldDecorator('legal_representative_idcard_under', {
                    initialValue: formData.legal_representative_idcard_under
                      ? [formData.legal_representative_idcard_under]
                      : '',
                    rules: [
                      {
                        required: true,
            ...
            <Row gutter={32}>
              <Col {...col}>
                <Form.Item label="行业资格证书">
                  <span style={{ color: 'red' }}>（图片上传格式jpg,jpeg,png）</span>
                  {getFieldDecorator('pra_qua_certificate', {
                    initialValue: formData.pra_qua_certificate
                      ? [formData.pra_qua_certificate]
                      : '',
                    rules: [
                      {
                        required: true,
