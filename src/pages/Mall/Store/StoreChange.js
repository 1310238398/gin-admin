...
/* 组件 */
export default class StoreChange extends React.PureComponent {
  /* 初始数据 */
  state = {
    checked: true,
    disabled: false,
  ...
  async componentDidMount() {
    ...
    if (!store) {
      ...
      if (store.jump === 2) {
        message.error('请先完善商铺信息');
        this.props.dispatch({
          type: 'store/redirectjump',
        });
      }
    }
  }
                ...
                <Col {...col}>
                  <Form.Item {...formItemLayout} label="营业开始时间">
                    {getFieldDecorator('open_time', {
                      ...
                      <TimePicker
                        defaultValue={moment('8:00', 'HH:mm')}
                        format="HH:mm"
                        disabled={this.state.disabled}
                      />
                    )}
          ...
          <Card bordered={false}>
            <Form>
              ...
              <Row>
                ...
                {/* <Col sm={9} push={4}>
                  <Form.Item label="法定代表人身份证(正反面)">
                    {getFieldDecorator('persional_id_image', {
                      initialValue: formData.persional_id_image
                        ? formData.persional_id_image.split(',')
                        : '',
                      rules: [
                        {
                          required: true,
                          message: '请上传法定代表人身份证(正反面)',
                        },
                      ],
                    })(<PicturesWall num={2} bucket="oper" listType="picture-card" />)}
                  </Form.Item>
                </Col> */}
              </Row>
