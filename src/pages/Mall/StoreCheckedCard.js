...
@Form.create()
export default class StoreCheckedCard extends PureComponent {
  componentDidMount() {
    const { id, type, callback } = this.props;
    this.props.dispatch({
      type: 'storeManage/checkedCardLoadForm',
      payload: {
        id,
        type,
        callback,
  ...
  render() {
    const {
      storeManage: { formTitle, formVisible, detailData, submitting },
