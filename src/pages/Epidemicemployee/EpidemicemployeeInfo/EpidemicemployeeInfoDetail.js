...
//  企业入驻的模态对话框组件。
export default class EpidemicemployeeInfoDetail extends PureComponent {
  state = {};
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      type: 'epidemicemployeeInfo/loadForm',
      payload: {
        id,
        type,
      },
  ...
  onModalCancelClick = () => {
    this.props.dispatch({
      type: 'epidemicemployeeInfo/changeFormVisible',
      payload: false,
    });
    this.props.callback();
  };
    ...
    return (
      <div className={styles.main}>
        {data.fill_in === 1 ? (
          ...
          <div className={styles.form} style={{ marginTop: 25 }}>
            <DescriptionList title="" size="large" col={3} style={{ marginBottom: 32 }}>
              <Description term="姓名">{data.name}</Description>
              <Description term="性别">{data.gender === '1' ? '男' : '女'}</Description>
              <Description term="企业名称">{data.enterprise_name}</Description>
            </DescriptionList>
            <DescriptionList title="" size="large" col={3} style={{ marginBottom: 32 }}>
              <Description term="证件类型">
                {this.renderStatus(data.certificate_type)}
              </Description>
              <Description term="证件号码">{data.license_num}</Description>
              <Description term="联系方式">{data.phone}</Description>
            </DescriptionList>
          </div>
        )}
      </div>
  ...
  render() {
    const {
      epidemicemployeeInfo: { formVisible, submitting },
    } = this.props;
