...
export default class NotifListCard extends React.PureComponent {
  state = {
    onItem: '1',
    arealoading: false,
    dataForm: false,
  ...
  sendOut = val => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        values.ranges = this.state.areaList;
        this.props.dispatch({
  ...
  onDataFormItemCallback = values => {
    const itemList = this.state.areaList;
    this.setState({ areaList: [...itemList,...values.ranges] });
  ...
  deleObj=(value)=>{
    ...
    const caveDataList = this.state.areaList;
    let ItemDataList = [];
    for(let i = 0; i<caveDataList.length;i+=1){
      if(value.bu_type===caveDataList[i].bu_type&&value.bu_id===caveDataList[i].bu_id) {
        ItemDataList = caveDataList.slice(i-1,1);
      }
    }
    this.setState({areaList:[...ItemDataList]});
  ...
  render() {
    const {
      notifList: { formTitle, formVisible, formData },
