  ...
  effects: {
    ...
    *queryShopStatueTotalInfo({ params, pagination }, { call, put, select }) {
      ...
      if (params) {
        ...
        const search = yield select(state => state.shopChangeaudit.search);
        if (search) {
          params = { ...search };
        }
      }
      if (pagination) {
        ...
        const pag = yield select(state => state.shopChangeaudit.pagination);
        if (pag) {
          pagination = { ...pag };
        }
      }
      const result = yield call(StoreService.queryModifyPageStore, {
        ...params,
        ...pagination,
      });
