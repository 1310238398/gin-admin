  ...
  effects: {
    *fetch({ payload, pagination }, { call, put, select }) {
      ...
      if (payload) {
        ...
        const search = yield select(state => state.boardInfo.search);
        if (search) {
          params = { ...search };
        }
      }
      ...
      if (pagination) {
        ...
        const pag = yield select(state => state.boardInfo.pagination);
        if (pag) {
          params = { ...params, ...pag };
        }
      }
      const orgid = yield select(state => state.boardInfo.orgid);
      params.org = orgid;
    ...
    *fetchFormExtra({ columnId, response }, { call, put, select }) {
      ...
      if (!formData) {
        formData = yield select(state => state.boardInfo.formData);
      }
    ...
    *submitDesc({ payload }, { call, put, select }) {
      ...
      const formID = yield select(state => state.boardInfo.formID);
      const orgid = yield select(state => state.boardInfo.orgid);
      params.org = orgid;
      if (formID) {
        // 修改
        ...
        if (response === 'ok') {
          ...
          const formCallback = yield select(state => state.boardInfo.formCallback);
          formCallback('ok');
        }
      } else {
        // 新增
