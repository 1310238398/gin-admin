  ...
  effects: {
    *fetch({ payload, pagination }, { call, put, select }) {
      ...
      if (payload) {
        ...
        const search = yield select(state => state.advertisPromotion.search);
        if (search) {
      ...
      if (pagination) {
        ...
        const p = yield select(state => state.advertisPromotion.pagination);
        if (p) {
          params = { ...params, ...p };
        }
      }
    ...
    *submit({ payload }, { call, put, select }) {
      ...
      const formID = yield select(state => state.advertisPromotion.formID);
      let response;
      if (formID && formID !== '') {
      ...
      if (response.record_id && response.record_id !== '') {
        ...
        const formCallback = yield select(state => state.advertisPromotion.formCallback);
        formCallback('ok');
      }
    },
